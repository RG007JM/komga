...
internal class KoboJapaneseEditionBridge(
  ...
  fun find(printIsbn: String): KoboProductLookupResult {
    val isbn = KoboLookupIsbn.normalize(printIsbn)
      ?: return failed("Japanese edition bridge requires a valid ISBN")
    requestCount.set(0)
    return try {
      val searchUrl = RAKUTEN_SEARCH.newBuilder().addQueryParameter("g", "000")
        .addQueryParameter("sitem", isbn).build()
      ...
      val candidate: RakutenEdition? = if (pairs.size == 1) {
      ...
      val edition = candidate ?: findVerifiedEdition(searchDocument, isbn, search, directPrint)
  ...
  private fun pairedSearchCards(document: Document): List<SearchPair> {
    ...
    for (anchor in document.select("a[href]")) {
      ...
      repeat(4) {
        ...
        val ebooks = wrapper.select("a[href]").mapNotNull { link ->
        ...
        if (ebooks.size == 1 && printUrls.singleOrNull() == print) {
          val titles = document.select("a[href]").filter { safeRakuten(it.attr("abs:href"), "rb") == print }
  ...
  private fun findVerifiedEdition(
    ...
    val prints = if (directPrint != null) listOf(directPrint) else
      searchDocument.select("a[href]").mapNotNull { safeRakuten(it.attr("abs:href"), "rb") }
        .distinct().take(MAX_PRINT_CANDIDATES)
    ...
    for (printUrl in prints) {
      ...
      val pairedLinks = if (primaryLinks.isEmpty() && directPrint == null) {
      ...
      for (ebookUrl in links) {
        ...
        if (sourceMetadata.publisher.isNotEmpty() && ebook.metadata.publisher.isNotEmpty() &&
          sourceMetadata.publisher != ebook.metadata.publisher) continue
        if (!direct && (sourceMetadata.publisher.isEmpty() || ebook.metadata.publisher.isEmpty() ||
            sourceMetadata.authors.isEmpty() || ebook.metadata.authors.isEmpty() ||
            sourceMetadata.authors.intersect(ebook.metadata.authors).isEmpty())) continue
  ...
  private fun editionLinks(source: Document): List<HttpUrl> =
    source.select("a[href]").filterNot(::inUnrelated).mapNotNull { link ->
      val ebook = safeRakuten(link.attr("abs:href"), "rk") ?: return@mapNotNull null
  ...
  private fun catalogueLinks(title: String): List<HttpUrl> {
    val request = RAKUTEN_SEARCH.newBuilder().addQueryParameter("g", "101")
      .addQueryParameter("sitem", title).build()
    val listing = fetchRakuten(request, "search")
    val document = Jsoup.parse(listing.html, listing.url.toString())
    val candidates = document.select("a[href]").filterNot(::inUnrelated).mapNotNull { link ->
  ...
  private fun readEbook(page: KoboWebsitePage): Ebook? {
    ...
    val ids = document.select("meta[property='books:isbn']").filterNot(::inUnrelated)
      .map { it.attr("content").trim() }.filter { BOOK_ID.matches(it) }.toSet()
    if (ids.size > 1) return null
    val id = ids.singleOrNull() ?: run {
      val primary = primaryInfo(document)
      BOOK_ID_LABEL.find(primary.text())?.groupValues?.get(1)?.filter(Char::isDigit)
        ?: document.select("#productDetailedDescription li.productInfo").firstNotNullOfOrNull { row ->
          val label = row.selectFirst(".category")?.text()?.trim()?.trim(':')?.lowercase(Locale.ROOT)
  ...
  private fun primaryPrintIsbn(document: Document): String? {
    ...
    document.select("[itemprop=isbn], meta[name=isbn], meta[property='book:isbn'], meta[property='books:isbn']")
      .filterNot(::inUnrelated).forEach { node ->
  ...
  private fun metadata(document: Document): Bibliography {
    ...
    val english = document.select("#productDetailedDescription li.productInfo").mapNotNull { row ->
      val label = row.selectFirst(".category")?.text()?.trim()?.trim(':')?.lowercase(Locale.ROOT)
      val value = row.selectFirst(".categoryValue")?.text()
      if (label == null || value == null) null else label to value
    }.toMap()
    val publisher = INFO_PUBLISHER.find(info)?.groupValues?.get(1)
      ?: english["publishers"] ?: english["publisher"].orEmpty()
    val authorText = INFO_AUTHORS.find(info)?.groupValues?.get(1)
      ?: english["authors"] ?: english["author"].orEmpty()
    val authors = authorText.split(',', '，', '、', '/', '／').map(::normalizeValue).filter(String::isNotEmpty).toSet()
  ...
  private fun primaryInfo(document: Document): Element {
    ...
    val panel = document.select("section, div")
      .filter { it != details && it.outerHtml().length < 16_000 &&
    ...
    val combined = if (scopes.isEmpty()) document.body().clone() else
  ...
  private fun verifyKoboJapan(printIsbn: String, ebookId: String): KoboProductLookupResult {
    // The opaque ebook ID is used ONLY here, never sent into the ISBN-only global search.
    val searchUrl = KOBO_JAPAN_SEARCH.newBuilder().addQueryParameter("query", ebookId)
      .addQueryParameter("pagenumber", "1").build()
    val search = fetchBounded(searchUrl)
    val direct = productUrl(search.url)
    val candidates = if (direct != null) listOf(direct) else {
      ...
      root.select("a[href*=/ebook/]").mapNotNull { link ->
    ...
    for (candidate in candidates.take(MAX_EBOOK_CANDIDATES)) {
      ...
      if (actual == null || !isJapaneseProduct(actual)) { uncertain = true; continue }
  ...
  private fun fetchRakuten(url: HttpUrl, kind: String): KoboWebsitePage {
  ...
  private fun safeRakuten(raw: String, kind: String): HttpUrl? {
  ...
  private fun productUrl(url: HttpUrl): HttpUrl? {
    ...
    if (url.scheme != "https" || url.host !in KOBO_HOSTS || path.size != 4 ||
      path[2] != "ebook" || path[3].isEmpty() || url.encodedPath.contains("%2f", ignoreCase = true)) return null
    return url.newBuilder().query(null).fragment(null).build()
  }

  private fun isJapaneseProduct(url: HttpUrl): Boolean = url.pathSegments.take(3) == listOf("jp", "ja", "ebook")
  private fun isJapaneseSearch(url: HttpUrl): Boolean =
    url.scheme == "https" && url.host in KOBO_HOSTS && url.encodedPath.trimEnd('/') == "/jp/ja/search"

  private fun inUnrelated(node: Element): Boolean = (node.parents() + node).any { ancestor ->
  ...
  private fun normalizeTitle(raw: String): String {
    ...
    return normalizeValue(title.replace(Regex("""\s*(?:\[?電子書籍版\]?|電子書籍)$"""), ""))
      .removePrefix("商品画像:").removePrefix("商品画像：")
  }

  private fun normalizeValue(raw: String) = Normalizer.normalize(raw, Normalizer.Form.NFKC)
  ...
  private data class SearchPair(val printUrl: HttpUrl, val ebookUrl: HttpUrl, val titleVariants: Set<String>)
  private data class Bibliography(val title: String, val publisher: String, val authors: Set<String>)
  private data class Ebook(val bookId: String, val metadata: Bibliography)
  private data class RakutenEdition(
    val bookId: String, val metadata: Bibliography, val printUrl: HttpUrl, val ebookUrl: HttpUrl,
