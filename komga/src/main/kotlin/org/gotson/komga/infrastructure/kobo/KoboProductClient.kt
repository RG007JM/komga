...
internal class KoboWebsiteHttpException(
  val statusCode: Int,
...
internal class KoboWebsiteBlockedException(
  val statusCode: Int,
...
class KoboProductClient(
  ...
  private val verifiedPages =
    object : LinkedHashMap<String, Triple<String, String, HttpUrl>>(128, 0.75f, true) {
  ...
  private val japaneseEditionBridge =
    KoboJapaneseEditionBridge(::getPage, pageParser) { isbn, ebookId, productId, url ->
      synchronized(verifiedPages) { verifiedPages["$isbn|$productId"] = Triple(ebookId, productId, url) }
    }
  private val websiteSearch =
    KoboWebsiteIsbnSearch(::getPage, pageParser, afterJapaneseMiss = japaneseEditionBridge::find) { isbn, productId, url ->
  ...
  internal fun localeForBook(bookId: String): String? =
    bookRepository
      ...
      ?.let(seriesMetadataRepository::findByIdOrNull)
      ?.language
        ...
        val store =
          found?.let {
            synchronized(verifiedPages) {
              verifiedPages["$normalized|${it.productId}"]
                ?.third
                ?.pathSegments
                ?.take(2)
                ?.joinToString("/")
