...
internal object KoboWebsiteStorefronts {
  val all =
    ("ca/en us/en in/en za/en au/en hk/zh jp/ja my/en nz/en ph/en sg/en tw/zh th/en " +
      ...
      "gb/en ar/es br/pt cl/es co/es mx/es pe/es ww/en").split(' ')

  private val byCountry = all.associateBy { it.substringBefore('/') }
  private val languageDefaults =
    mapOf(
      "cs" to "cz/cs", "da" to "dk/da", "de" to "de/de", "el" to "gr/en",
      "en" to "gb/en", "es" to "es/es", "et" to "ee/en", "fi" to "fi/fi",
      "fr" to "fr/fr", "it" to "it/it", "lt" to "lt/en", "nb" to "no/nb",
      "nl" to "nl/nl", "pl" to "pl/pl", "pt" to "pt/pt", "ro" to "ro/ro",
      "sk" to "sk/en", "sv" to "se/sv", "tr" to "tr/tr",
    )

  /** Region is read only from the *core* BCP-47 tag, never extensions or private use. */
  fun second(locale: String?): String {
    val tag = locale?.trim()?.replace('_', '-')?.lowercase().orEmpty()
