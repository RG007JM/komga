...
class KoboWebsiteIsbnSearchTest {
  ...
  private fun page(
    ...
    bookProduct: String = product,
    bookSeries: String? = series,
  ): String =
    """
  ...
  fun `worldwide miss tries selected region and never checks other stores after match`() {
    ...
    val lookup =
      KoboWebsiteIsbnSearch({ url ->
        calls += url.encodedPath
        if (url.encodedPath.startsWith("/ww/"))
          KoboWebsitePage(url, "<h2 data-testid='no-result'>No</h2>")
        else
          KoboWebsitePage("https://www.kobo.com/it/it/ebook/italian-book".toHttpUrl(), page("it/it", bookSeries = null))
      }, parser)
  ...
  fun `a cross storefront redirect cannot claim a match in its requested storefront`() {
    ...
    val lookup =
      KoboWebsiteIsbnSearch({ url ->
        calls += url.encodedPath
        if (url.encodedPath.startsWith("/ww/"))
          KoboWebsitePage("https://www.kobo.com/ca/en/ebook/book".toHttpUrl(), page("ca/en"))
        else
          KoboWebsitePage(url, "<h2 data-testid='no-result'>No</h2>")
      }, parser)
    assertThat(lookup.find(isbn, "ja-JP")).isInstanceOf(KoboProductLookupResult.Failed::class.java)
  ...
  fun `one indeterminate storefront prevents a durable negative even if others explicitly miss`() {
    ...
    val lookup =
      KoboWebsiteIsbnSearch({ url ->
        calls += url.encodedPath
        if (url.encodedPath == "/ww/en/search")
          KoboWebsitePage(url, "<main>Unknown layout</main>")
        else
    ...
    val lookup =
      KoboWebsiteIsbnSearch({ url ->
        ...
        if (url.encodedPath.startsWith("/ww/"))
          ...
          KoboWebsitePage("https://www.kobo.com/jp/ja/ebook/voy_BUz4-D2yFrMo73mJsQ?sId=secret".toHttpUrl(), page("jp/ja"))
      }, parser)
